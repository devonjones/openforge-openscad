...
export function useImports(urls?: ({ url: string; name?: string })[], autoImport = false) {
    ...
    (async () => {
      ...
      try {
        ...
        if (urls && Array.isArray(urls)) {
          nfiles = urls.map(u => {
            return { name: u.name || u.url.split('/').pop() || 'file', url: u.url };
          });
        } else {
          nfiles = [];
        }
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        let nfiles;
        if (url.startsWith('/')) {
          // Local relative path, fetch directly
          nfiles = [
            {
